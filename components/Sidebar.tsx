      ...
      <aside className="w-64 bg-white border-r border-slate-100 flex flex-col h-screen sticky top-0 hidden md:flex">
        ...
        <div className="px-6 mb-8">
          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
             <div className={`w-8 h-8 ${currentUser?.avatarColor || 'bg-slate-300'} rounded flex items-center justify-center text-white text-[10px] font-bold shrink-0`}>
        ...
        <div className="p-6 border-t border-slate-50">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Cloud Ativa</p>
          </div>
        </div>
      </aside>

      {/* Bottom Navigation Mobile (PWA style) */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 flex items-center justify-around px-2 py-3 md:hidden z-40 pb-safe shadow-[0_-1px_15px_rgba(0,0,0,0.05)]">
        {menuItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onViewChange(item.id as ViewType)}
            className={`flex flex-col items-center gap-1.5 flex-1 transition-all py-1 ${
          ...
          >
            ...
            <span className={`text-[8px] font-black uppercase tracking-tighter ${currentView === item.id ? 'opacity-100' : 'opacity-70'}`}>
              {item.shortLabel}
            </span>
            {currentView === item.id && (
