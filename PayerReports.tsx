    ...
    <div className="max-w-2xl space-y-6">
      <div className="bg-white p-6 md:p-8 rounded-lg border border-slate-100 shadow-sm">
        ...
        <form onSubmit={handleAdd} className="space-y-4 mb-10 pb-10 border-b border-slate-50">
          ...
          <button 
            type="submit"
            className="w-full bg-indigo-600 text-white py-3.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 active:scale-[0.98]"
          >
            Adicionar Categoria
          </button>
        </form>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {categories[activeType].map((cat) => {
            ...
            if (isEditing) {
              return (
                <div key={cat} className="flex flex-col gap-4 p-4 bg-indigo-50/20 rounded-lg border border-indigo-100 animate-in fade-in duration-200 col-span-1 sm:col-span-2">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input 
                      type="text" 
                      value={editValue} 
                      onChange={e => setEditValue(e.target.value)}
                      className="w-full px-3 py-2 bg-white border border-indigo-100 rounded-md text-xs font-bold text-slate-700 outline-none"
                    />
                    <input 
                      type="color" 
                      value={editColor} 
                      onChange={e => setEditColor(e.target.value)}
                      className="w-full h-[36px] p-0.5 bg-white border border-indigo-100 rounded-md cursor-pointer"
                    />
                  </div>
                  
                  <div className="flex flex-wrap gap-1.5 p-2 bg-white/50 rounded-md border border-indigo-50">
                    {Object.keys(AVAILABLE_ICONS).map(iconKey => (
                      <button
                        key={iconKey}
                        type="button"
            ...
            return (
              <div key={cat} className="flex items-center justify-between px-4 py-3 rounded-lg bg-slate-50/50 hover:bg-slate-50 border border-transparent hover:border-indigo-50 group transition-all">
                <div className="flex items-center gap-3">
                  <div 
                    className={`w-9 h-9 rounded-lg flex items-center justify-center border shrink-0 ${!styles.isCustom ? `${styles.bg} ${styles.text} ${styles.border}` : ''}`}
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                  <button 
                    ...
                    className="p-1.5 text-slate-300 hover:text-indigo-600 hover:bg-white rounded-md transition-all"
                  >
                  ...
                  <button 
                    ...
                    className="p-1.5 text-slate-300 hover:text-rose-500 hover:bg-white rounded-md transition-all"
                  >
